...
double parse_dt(string str)
{
    ...
    cmatch matches;
    if (!regex_search(str.c_str(), matches, pattern))
        return 0.0;
...
inline static void process_block(const char *buf, uint32_t *digest)
{
    ...
    {
        block[i / 4] = (buf[i + 3] & 0xFF) | (buf[i + 2] & 0xFF) << 8 | (buf[i + 1] & 0xFF) << 16 | (buf[i + 0] & 0xFF) << 24;
    }
